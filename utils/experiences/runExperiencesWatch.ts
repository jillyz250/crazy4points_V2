...
async function parseChunk(anthropic: Anthropic, chunk: string, program: ExperienceProgram): Promise<ParsedListing[]> {
  const msg = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Extract every distinct experience listing in this fragment of a ${program.source_platform} page. Return ONLY a JSON array; one object per listing with exactly these keys: {"title":string,"points":int|null (current bid or redeem points; null if it is cardmember access/presale paid with cash),"format":"bid"|"redeem"|"access" ("access" = cardmember presale/access bought with a card, not points),"category":string|null (music/sports/entertainment/etc),"location":string|null (the CITY and its region/country of the experience, e.g. "London, UK" or "New York, NY" - read it from the text, or infer it from a clearly-known venue/team/festival named in the title, e.g. "Madison Square Garden"->"New York, NY", "Atlanta Hawks"->"Atlanta, GA", "Austin City Limits"->"Austin, TX"; use null ONLY when the city is genuinely indeterminable, never guess a city you are unsure of),"event_date":string|null,"detail_url":string|null (the listing's own page URL if present),"ended":boolean (true if this listing shows it has FINISHED - e.g. "Auction ended", "Winning Bid", "View history", "Sold Out", or a clearly past deadline - else false)}. Skip navigation, footer, category headers, and promo banners. If the fragment has no listings, return [].\n\n${chunk}`,
      },
    ],
  })
  try {
