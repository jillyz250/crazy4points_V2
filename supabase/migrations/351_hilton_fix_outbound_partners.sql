...
--
-- HISTORY NOTE: This was originally migration 350 on the
-- hilton-source-doc-and-slug-fix branch, but PR #818 merged before that
-- commit was pushed. The data was applied to prod DB manually via
-- `supabase db query` on 2026-05-26 (Hilton outbound table is correct in
-- the live DB). This migration re-files the file in the repo so a future
-- DB rebuild from migrations produces the same state.
--
-- ────────────────────────────────────────────────────────────────────────
--
-- Migration 346 included only 5 representative airlines (and incorrectly
-- listed American Airlines + Alaska as partners — AA was never a Hilton
-- partner, and Alaska was dropped along with Aeroflot/Avianca/Finnair/
-- Frontier/Garuda/Hawaiian/LATAM/Philippine/South African in June 2024).
-- This patch replaces the outbound JSONB with the current full 24-partner
-- list including:
...
-- (verified May 26, 2026), cross-referenced against Award Travel Finder
-- 2026 and NerdWallet 2026. Singapore KrisFlyer 8:1 ratio independently
-- confirmed via Singapore Airlines' own info page after NerdWallet outlier
-- (20:1) was detected — see plans/sources/hilton.md fact-check log.
--
-- InterMiles (Jet Airways) is a Hilton partner but not seeded in our
-- programs table — captured as a TODO in plans/sources/hilton.md.
--
-- Editorial note: hotel-to-airline transfers from Hilton are almost never
-- the right move - your Hilton points are typically worth more as Hilton
-- stays than as airline miles. Keeping this list complete for reference,
-- not because we'd recommend most of these transfers.
