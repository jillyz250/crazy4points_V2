-- Phase 3 Wave 2 — trigger preserves `primary_program_id` on topic.metadata.
--
-- Wave 2 flips #7–#10 (daily brief index, daily brief archive, /alerts list,
-- /programs/[slug] alerts archive) all render alert cards that distinguish
-- primary vs secondary programs. topics.programs[] is just an array of
-- slugs with no role info, so we need the primary id stored separately for
-- the adapter to reconstruct the alert_programs shape correctly.
--
-- Only one new line in jsonb_build_object — everything else unchanged from
-- migration 321. CREATE OR REPLACE keeps the trigger active without a gap.
...
BEGIN
  ...
  v_topic_metadata := jsonb_build_object(
    ...
    'primary_program_id', NEW.primary_program_id
  );
