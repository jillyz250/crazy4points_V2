-- Phase 3 Wave 2 — trigger correction. The Wave 1 trigger preserved most of
-- the alerts shape but missed a handful of fields the Wave 2 read-path
-- adapter needs to reconstruct the full Alert shape:
--
--   • short_slug              → variant.metadata.short_slug
--   • start_date              → variant.metadata.start_date
--   • alert.source            → variant.metadata.alerts_source (renamed to
--                               avoid colliding with the provenance marker
--                               metadata.source = 'alerts_dual_write')
--   • alert.type              → variant.metadata.original_alert_type
--                               (topic.topic_type is a smaller enum; values
--                               like 'point_purchase' fall through to 'other'
--                               so we preserve the original on the variant)
--   • alert.created_at        → topic.metadata.original_alert_created_at
--   • alert.updated_at        → topic.metadata.original_alert_updated_at
--
-- Replaces the function from migration 318 in place — CREATE OR REPLACE so
-- the trigger keeps firing. No data change yet; new field values land on
-- existing rows the next time their alert is updated. Phase 3 Wave 2 also
-- re-runs the backfill script to fill these onto the 62 existing variants
-- immediately.
...
BEGIN
  ...
  -- alert.source renamed to alerts_source to avoid collision with provenance marker.
  -- alert.type preserved as original_alert_type so the adapter can return the full
  -- value (topic.topic_type is a subset).
