#!/usr/bin/env python3
"""
Card "Companion" - fillable, printable benefits checklist (generic renderer).
Built for crazy4points.com. Run: python build_companion.py --card <name>
Card data lives in cards/<name>.py; this file is the layout/renderer only.
"""
import os
...
_ap.add_argument("--card", default="csr", help="card data module in cards/")
_args, _ = _ap.parse_known_args()
_cardmod = importlib.import_module(f"cards.{_args.card}")
# inject the card's data (UPPERCASE names + CARD) into this module's globals
globals().update({k: v for k, v in vars(_cardmod).items()
                  if k.isupper() or k == "CARD"})
OUT = os.path.join(HERE, CARD["out_name"])
...
ICONS = {"peloton": rc_peloton, "ticket": rc_ticket, "bag": rc_bag, "crown": rc_crown,
         "key": rc_key, "phone": rc_phone, "passport": rc_passport, "tv": rc_tv,
         "music": rc_music, "dine": rc_dine}
...
def draw_setup_grid(y):
    ...
    for i, item in enumerate(GRID):
        ...
        ICONS[item["icon"]](cx0 + tw - 18, cy0 - 16, alpha(PURPLE, 0.62))
...
def mini_header(pageno):
    ...
    text(MARGIN, y - 23, CARD["mini_title"], font="Head", size=13, col=white)
...
def draw_cta(y):
    ...
    text(x + 52, y - 30, CARD["cta_headline"], font="Head", size=17, col=white)
    text(x + 52, y - 52, CARD["cta_sub1"],
         font="Body", size=9.5, col=alpha(white, 0.92))
    text(x + 52, y - 66, CARD["cta_sub2"],
...
def draw_total(y):
    ...
    text(tx + tfw + 8, fy + 6, CARD["tracker_total"], font="UIB", size=11, col=GOLD_D)
