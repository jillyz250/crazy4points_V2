...
From the snapshot, pick the chart-worthy candidates across the actionable queues: pending drafts, intel to triage, transfer-data changes, **welcome-bonus changes**, prose-to-recheck, the **refresh queue** (surface the oldest few due), **new experiences** (marquee listings worth a post), and **sweepstakes post candidates** (the snapshot ranks unposted running sweeps by content + soonest deadline; ⭐ = the day's best social pick — points/miles giveaways lead, Jill's best-performing format). Checking sweepstakes + experiences for a social/alert is part of the morning (Jill asked, 2026-08-11) — the daily social post usually comes from here. **Verify each candidate against its official issuer/program source BEFORE you build the chart** (Step 1b is the how) — never present a chart of unverified claims. Then build **ONE table**, ordered by leverage.

Columns: **Item · What it is · Reverified · Pages correct · My rec · Social?** — the two check columns are REQUIRED on every row (Jill asked for them explicitly, 2026-08-11).
- **Reverified** — ✅ re-confirmed against the OFFICIAL issuer/program source this morning (not memory, not a blog), or ⚠️ can't verify (targeted/personalized offer, or source unreachable → cannot publish as a general alert). This is the checkbox that catches the "6-year-old stale news" and "expired promo" traps.
- **Pages correct** — the state of the tied program/card page(s) AFTER any fix. **✅ correct** (page verified accurate, or fixed as part of publishing), **✍️ needs fixing** (name the field — do it before you call the item done), or **—** (n/a). A published alert isn't finished until this is ✅ ([[feedback_cross_check_alerts_vs_program_pages]]).
