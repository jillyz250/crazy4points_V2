...
From the snapshot, pick the chart-worthy candidates across the actionable queues: pending drafts, intel to triage, transfer-data changes, **welcome-bonus changes**, prose-to-recheck, the **refresh queue** (surface the oldest few due), and **new experiences** (marquee listings worth a post; the snapshot's Experiences section + count). **Verify each candidate against its official issuer/program source BEFORE you build the chart** (Step 1b is the how) — never present a chart of unverified claims. Then build **ONE table**, ordered by leverage.
