...
- **⭐ Monetize when relevant (Jill, 2026-09-04).** When a card/program we cover has a way to earn (affiliate/referral), surface it — prefer our monetized link, fall back to the issuer/official page, never a dead end. The rail: card-bonus alerts set `apply_card_slug`; `components/alerts/ApplyHere.tsx` renders the compliant "Apply here" button (affiliate → `rel="nofollow sponsored"` + disclosure; issuer → plain). A card we cover with NO affiliate link is unrealized revenue → flag Megan (partnerships). Monetization never bends coverage or the honest take.
