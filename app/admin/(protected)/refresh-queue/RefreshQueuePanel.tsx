...
/** Live total count of items due for re-verification — for the hub tab badge. */
export async function refreshQueueCount(): Promise<number> {
  const supabase = createAdminClient()
  const byType = await getRefreshQueueByType(supabase)
  return Object.values(byType).reduce((s, n) => s + n, 0)
}

/**
 * Refresh Queue panel — the body of the old /admin/refresh-queue page, relocated
 * to render inside the Accuracy hub. `type` is the active filter; `basePath` is
 * where the filter chips link (the hub tab by default, so filtering stays inside
 * the hub).
 */
export default async function RefreshQueuePanel({
  type,
  basePath = '/admin/accuracy?tab=refresh-queue',
}: {
  type?: string
  basePath?: string
}) {
  ...
  // The base path may already carry a query string (the hub tab); pick the right
  // joiner so a type filter appends cleanly.
  const withType = (t?: string) => (t ? `${basePath}${basePath.includes('?') ? '&' : '?'}type=${encodeURIComponent(t)}` : basePath)
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1.25rem' }}>
        <Link href={withType()} style={chipStyle(!type)}>
          All {total}
        </Link>
        {Object.entries(byType).sort((a, b) => b[1] - a[1]).map(([t, n]) => (
          <Link key={t} href={withType(t)} style={chipStyle(type === t)}>
            {typeLabel(t)} {n}
          </Link>
        ))}
      </div>
